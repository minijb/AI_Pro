...
def validate_skill(skill_path):
    ...
    for field in ('disable-model-invocation', 'user-invocable'):
        if field in frontmatter and not isinstance(frontmatter[field], bool):
            return False, f"'{field}' must be a boolean (true/false), got {type(frontmatter[field]).__name__}"

    # Validate effort field
    effort = frontmatter.get('effort')
    if effort is not None and effort not in ('low', 'medium', 'high', 'max'):
        return False, f"'effort' must be one of: low, medium, high, max. Got '{effort}'"
    ...
    if shell is not None and shell not in ('bash', 'powershell'):
        return False, f"'shell' must be 'bash' or 'powershell'. Got '{shell}'"
